...
function doRectanglesOverlap(rect1, rect2) {
  const leftOverlap = rect1.left < rect2.left + rect2.width;
  const rightOverlap = rect1.left + rect1.width > rect2.left;
  const topOverlap = rect1.top < rect2.top + rect2.height;
  const bottomOverlap = rect1.top + rect1.height > rect2.top;
...
function isInsideCircle(circle, point) {
  ...
  const distanceCenter = Math.sqrt(distanceX ** 2 + distanceY ** 2);
  return distanceCenter < circle.radius;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  // Remove the trailing slash if the common path is not empty
  if (commonPath !== '') {
...
function getMatrixProduct(m1, m2) {
  ...
  if (m1[0].length !== m2.length) {
    throw new Error('Invalid matrices dimensions for multiplication.');
  }
